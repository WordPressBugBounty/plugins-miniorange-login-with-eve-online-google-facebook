...
class MO_OAuth_Client_Support {
	...
	public static function mo_usecase_page() {
		...
		<div class="mo_oauth_usecase_discription_container">
				<div class="mo_oauth_usecase_discription_main_card mo_oauth_x_support_1">
					<button onclick="mo_oauth_show_usecases()" class="mo_oauth_usecase_discription_title">
					<h2 class="mo_oauth_x_support_2"><u>
					<?php
					echo esc_html( 'Checkout our popular usecases for ' . ucwords( $configuredapp ) );
					?>
					</u>
					</h2>
					<i class="fa fa-chevron-down mo_oauth_x_support_3" id="mo_oauth_usecase_down_arrow"></i>
					</button>
					<div id="mo_oauth_usecase" class="mo_oauth_usecase mo_oauth_x_support_4">
					<?php
					...
					?>
					<div class="mo_oauth_usecase_link_container mo_oauth_x_support_10" id="mo_oauth_usecase_link_container">
					<?php if ( ! empty( $link ) ) { ?>
					<a class="mo_oauth_instruction_btn mo_oauth_instruction mo_oauth_usecase_btn" id="mo_oauth_usecase_link" href="<?php echo esc_attr( $link ); ?>" target="_blank" rel="noopener">KNOW MORE</a>
					<?php } ?>
					</div>
				</div>
				</div>
...
<div id="mo_support_layout" class="mo_support_layout mo_oauth_outer_div">
	<div>
		...
		<form method="post" action="" id="mo-contact-us-form">
	...
	<div class="mo_oauth_contact">
			<table class="mo_settings_table mo_oauth_x_support_7">
				...
				<textarea cols="30" rows="6"
					...
					onkeyup="mo_oauth_valid_query(this)" rows="4" class="mo_oauth_x_support_9" onblur="mo_oauth_valid_query(this)" required></textarea>
				...
				<div class="mo_oauth_setup-call">
					<div class="form-check form-switch">
						<input class="form-check-input" type="checkbox" role="switch" id="oauth_setup_call" name="oauth_setup_call" />
						<label class="form-check-label" for="oauth_setup_call"><?php esc_html_e( 'Setup a Call/ Screen-share session', 'miniorange-login-with-eve-online-google-facebook' ); ?></label>
					</div>
				</div>
				<div id="mo_oauth_setup_call_div">
					<table class="mo_settings_table" cellpadding="2" cellspacing="2">
						...
						<tr id="setup_guide_link" class="mo_oauth_x_support_7">
							<td colspan="2">
						...
						<tr id="required_mark" class="mo_oauth_x_support_7">
		...
		<form id="mo_oauth_enable_debug_log_form" method="post">
			<input type="hidden" name="option" value="mo_oauth_reset_debug" />
				<?php wp_nonce_field( 'mo-oauth-Debug-logs-unique-string-nonce', 'mo_oauth_reset_debug' ); ?>
					<div class="form-check form-switch">
						<input class="form-check-input" type="checkbox" role="switch" id="mo_oauth_debug_check" name="mo_oauth_debug_check"
						<?php
						if ( get_option( 'mo_debug_enable' ) ) {
							echo get_option( 'mo_debug_enable' ) === 'on' ? 'checked' : 'unchecked';}
						?>
						>
						<label class="form-check-label" for="mo_oauth_debug_check"><?php esc_html_e( 'Enable Debug Log', 'miniorange-login-with-eve-online-google-facebook' ); ?></label>
					</div>
			...
			<form id="mo_oauth_debug_download_form" method="post">
				...
				<button type="submit" name="submit" value="Download Logs" class="mo_oauth_download-logs-btn">
					<span class="dashicons dashicons-download" aria-hidden="true"></span>
					<?php esc_html_e( 'Download Logs', 'miniorange-login-with-eve-online-google-facebook' ); ?>
				</button>
			</form>
			...
			<?php wp_nonce_field( 'mo_oauth_clear_debug', 'mo_oauth_clear_debug_nonce' ); ?>
				...
				<button type="submit" name="submit" id="submit_clear" value="Clear Logs" class="mo_oauth_clear-logs-btn">
					<span class="dashicons dashicons-trash" aria-hidden="true"></span>
					<?php esc_html_e( 'Clear Logs', 'miniorange-login-with-eve-online-google-facebook' ); ?>
				</button>
			</form>
